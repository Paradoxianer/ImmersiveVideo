	...
	const media_format& format)
{
	...
	err = AllocateOutputBufferGroup();
	if (err != B_OK)
		ERROR("\tAllocateInputBufferGroup: %s\n",strerror(err));
	int32 changeTag;
	if (filterOutput.destination!=media_destination::null)
		ChangeFormat(filterOutput.source,filterOutput.destination,&(filterInput.format));
	return err;
}
...
VideoCopyFilter::NodeRegistered()
{
	...
	filterOutput.destination = media_destination::null;
	strcpy(filterOutput.name, "VideoCopyFilter Output");	
...
void VideoCopyFilter::CalcInt32(int32 *inbuffer, int32* outbuffer, uint64 size)
{
...
void VideoCopyFilter::CalcInt16(int16 *inbuffer, int16* outbuffer, uint64 size)
{
...
void VideoCopyFilter::CalcInt8(int8 *inbuffer, int8* outbuffer, uint64 size)
{
...
void VideoCopyFilter::NoCalc(uint32 *inbuffer, uint32* outbuffer, uint64 size)
{
...
/*		if (err != B_OK)
			ERROR("\tERROR - SetOutputBuffersFor(NULL) = %s", strerror(err));
		err = inputBufferGroup->ReclaimAllBuffers();
		if (err != B_OK)
			ERROR("\tERROR - inputBufferGroup->ReclaimAllBuffers() = %s", strerror(err));
		err = inputBufferGroup->WaitForBuffers();
		if (err != B_OK)
			ERROR("\tERROR - inputBufferGroup->WaitForBuffers() = %s", strerror(err));
...
VideoCopyFilter::AllocateOutputBufferGroup()
{
	...
	status_t err=B_OK;
	/*if ((outputBufferGroup)&&(ownBufferGroup))
	{
		if (err != B_OK)
			ERROR("\tERROR - SetOutputBuffersFor(NULL) = %s", strerror(err));
		err = outputBufferGroup->ReclaimAllBuffers();
		if (err != B_OK)
			ERROR("\tERROR - inputBufferGroup->ReclaimAllBuffers() = %s", strerror(err));
		err = outputBufferGroup->WaitForBuffers();
		if (err != B_OK)
			ERROR("\tERROR - inputBufferGroup->WaitForBuffers() = %s", strerror(err));
		delete outputBufferGroup;
	}*/
...
void VideoCopyFilter::HandleBufferWrap( BBuffer *inBuffer, bigtime_t lateness )
{
	...
	if( (destinationID==filterInput.destination.id) && running )
	{
		bigtime_t start = TimeSource()->RealTime();
			...
			if (err!=B_OK)			
			{	
				ERROR("ERROR - SendBuffer = %s",strerror);
				outBuffer->Recycle();
			}
