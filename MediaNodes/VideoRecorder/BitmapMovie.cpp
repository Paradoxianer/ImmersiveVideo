...
status_t 
BitmapMovie::WriteFrame(void* bitmap, bool isKeyFrame)
{
...
/*	if (!bitmap) return B_BAD_VALUE;
	...
	{
/*		printf("Width: Bitmap %d, Movie %d\n",r.Width() , mWidth-1);
		printf("Height: Bitmap %d, Movie %d\n",r.Height() , mHeight-1);
...
//	}
	...
	if (!err) err = mTrack->WriteFrames(bitmap, 1, (isKeyFrame) ? B_MEDIA_KEY_FRAME : 0);
	return err;
}
