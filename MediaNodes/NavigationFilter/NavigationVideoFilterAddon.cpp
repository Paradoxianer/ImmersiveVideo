...
#include "NavigationVideoFilterAddon.h"
	...
	: BMediaAddOn(imid)
{
	...
	fFlavorInfo.name = "NavigationVideoFilter";
		...
		const flavor_info *info, BMessage *config, status_t *out_error)
{
	...
	node = new NavigationVideoFilter(this, fFlavorInfo.name, fFlavorInfo.internal_id);
