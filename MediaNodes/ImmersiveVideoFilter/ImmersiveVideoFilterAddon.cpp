...
#include "ImmersiveVideoFilterAddon.h"
	...
	: BMediaAddOn(imid)
{
	...
	fFlavorInfo.name = "ImmersiveVideoFilter";
		...
		const flavor_info *info, BMessage *config, status_t *out_error)
{
	...
	node = new ImmersiveVideoFilter(this, fFlavorInfo.name, fFlavorInfo.internal_id);
