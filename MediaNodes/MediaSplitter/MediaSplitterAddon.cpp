	...
	: BMediaAddOn(imid)
{
	...
	fFlavorInfo.name = "MediaSplitter";
	fFlavorInfo.info = "A Filter to split a MediaStream";
		...
		const flavor_info *info, BMessage *config, status_t *out_error)
{
	...
	node = new MediaSplitter(this, fFlavorInfo.name, fFlavorInfo.internal_id);
