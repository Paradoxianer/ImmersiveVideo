...
PluginManager::PluginManager(ImmersiveVideoFilter *iVFilter):BLooper()
{
...
uint32	*PluginManager::GetLookUpTableInt(void)
{
	...
	uint32 size=lTableSize*sizeof(uint32)*2;
	...
	for (int i=0;i<lTableSize;i++)
	 	lTable[i]=((uint32)tmpLookUp[i].x+((uint32)tmpLookUp[i].y*iDim->IntegerWidth()));
	Unlock();
	return lTable;	
}
